...
def rendering(model, rays_origin, rays_direction, tn, tf, nb_bins = 100, device = 'cpu', white_background = True):
    ...
    delta = torch.cat((t[1:] - t[:-1], torch.tensor([1e10], device=device))) #size is no of bins - 1 so we concat infinity as the last value, most nerf papers take the last delta value as infinity
