...
def ray_generator(
    height: int, width: int, focus: int, camera2world=None
...
def get_ray_directions(height: int, width: int, focus: int) -> np.ndarray:
    ...
    directions: np.ndarray = np.stack(
        (x, -y, -z), axis=-1  # -ve as we want y downwards
    )  # -ve as were looking down throught -z axis
...
def get_rays(
    height: int, width: int, directions: np.ndarray, camera2world: np.ndarray = None
):
    rays_origin: np.ndarray = np.zeros((height * width, 3))
    rays_direction: np.ndarray = np.zeros((height * width, 3))
